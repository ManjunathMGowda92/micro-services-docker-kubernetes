    ...
    public RouteLocator bankRouteLocator(RouteLocatorBuilder builder) {
        return builder
                ...
                .route(apiPath -> apiPath
                        .path("/bank-app/loans/**")
                        .filters(api -> api.rewritePath("/bank-app/loans/info", "/loans-service/loans/info")
                                .rewritePath("/bank-app/loans/contact-info", "/loans-service/loans/contact-info")
                                .rewritePath("/bank-app/loans", "/loans-service/api/v1/loans")
                                .rewritePath("/bank-app/loans/(?<segment>.*)", "/loans-service/api/v1/loans/${segment}")
                                .addResponseHeader("X-Response-Time", LocalDateTime.now().toString())
                                .retry(retryConfig -> retryConfig.setRetries(4)
                                        .setBackoff(Duration.ofMillis(500), Duration.ofMillis(2000), 2, true)
                                        .setMethods(HttpMethod.GET))
                ...
                .route(apiPath -> apiPath.path("/bank-app/cards/**")
                        .filters(api -> api.rewritePath("/bank-app/cards/info", "/cards-service/cards/info")
                                .rewritePath("/bank-app/cards", "/cards-service/api/v1/cards")
