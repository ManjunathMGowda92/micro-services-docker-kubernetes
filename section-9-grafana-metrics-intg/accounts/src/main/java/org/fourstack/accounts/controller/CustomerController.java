...
public class CustomerController {
    ...
    public ResponseEntity<CustomerDto> retrieveDetailsByMobileNumber(
            @PathVariable @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
            String mobileNumber) {
        logger.info("Retrieve Customer details by mobile number : {}", mobileNumber);
        return customerService.retrieveCustomerByMobileNumber(mobileNumber);
    }

    public ResponseEntity<CustomerDto> retrieveDetailsByMobileNumberFallback(String mobileNumber, Throwable throwable) {
        logger.error("Executing Fallback mechanism for retrieveDetailsByMobileNumber:{} due to exception",
                mobileNumber, throwable);
